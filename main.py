...
def repair_existing_order_messages():
    # Normalize legacy orders to: photos/receipt first, then ONE details+buttons message.
    changed=False
    for o in DATA.get('orders',[]):
        mids=[m for m in (o.get('channel_message_ids') or []) if m]
        if not mids or not ORDERS_CHAT: continue
        typ=o.get('type')
        if o.get('ref_photo') and o.get('receipt') and typ in {'customer','logo','normal'} and len(mids)>=2:
            # Remove any old captions from the two album messages.
            for mid in mids[:2]:
                api('editMessageCaption',{'chat_id':ORDERS_CHAT,'message_id':mid,'caption':'','reply_markup':None})
            # Delete all old text/action messages after the album and recreate one canonical action message.
            for mid in mids[2:]:
                api('deleteMessage',{'chat_id':ORDERS_CHAT,'message_id':mid})
            r=send(ORDERS_CHAT,view_order_text(o),order_action_kb(o.get('id')))
            if r.get('ok'):
                am=r.get('result',{}).get('message_id'); o['channel_action_message_id']=am; o['channel_message_ids']=mids[:2]+[am]; changed=True
        elif len(mids)>=1:
            # For legacy single-photo/text orders, create a clean details message if needed.
            target=o.get('channel_action_message_id')
            if target and target in mids:
                if edit_order_message(ORDERS_CHAT,target,o): changed=True
            else:
                # Treat the last message as the old details/action message. Clear its caption if it is a photo.
                last=mids[-1]
                api('editMessageCaption',{'chat_id':ORDERS_CHAT,'message_id':last,'caption':'','reply_markup':None})
                r=send(ORDERS_CHAT,view_order_text(o),order_action_kb(o.get('id')))
                if r.get('ok'):
...
def finish_order(uid):
    ...
    if ORDERS_CHAT:
        ...
        # Order channel layout: photos first, then ONE details message with the action buttons.
        # Never put order details in an album caption, because that caused duplicate details
        # and made the action buttons unreliable on legacy orders.
        ids=[]
        if s.get('ref_photo') and s.get('receipt'):
            ...
            r=send_photo(ORDERS_CHAT,s['receipt'],'')
            if r.get('ok'): ids=[r.get('result',{}).get('message_id')]
        if ids:
            r2=send(ORDERS_CHAT,details,order_action_kb(oid))
            if r2.get('ok'): o['channel_action_message_id']=r2.get('result',{}).get('message_id'); ids.append(o['channel_action_message_id'])
        ...
        if step=='receipt':
            ...
            if ORDERS_CHAT:
                ...
                if s.get('receipt'):
                    r=send_photo(ORDERS_CHAT,s['receipt'],'')
                    if r.get('ok'): ids=[r.get('result',{}).get('message_id')]
                r2=send(ORDERS_CHAT,vip_details,order_action_kb(oid))
                if r2.get('ok'):
...
def callback(uid,cid,data,msgid=None):
    ...
    if data.startswith('pay_ok|') or data.startswith('pay_bad|'):
        ...
        if not ok:
            ...
            edit_order_message(cid,o.get('channel_action_message_id') or msgid,o) or send(cid,view_order_text(o),order_action_kb(oid))
