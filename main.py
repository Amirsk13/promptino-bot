...
def publish_post(uid):
    ...
    links=kb([[urlbtn(b['label'],f'https://t.me/{BOT_USERNAME}?start={pid}_{b["key"]}')] for b in s['buttons']]) if BOT_USERNAME else None
    # Telegram media groups cannot carry inline keyboards, so with multiple photos send media first then text/buttons.
    if len(s['photos'])==1: send_photo(PROMPTINO_CHAT,s['photos'][0],post_preview(s),links)
    else:
        api('sendMediaGroup',{'chat_id':PROMPTINO_CHAT,'media':[{'type':'photo','media':p} for p in s['photos']]})
        send(PROMPTINO_CHAT,post_preview(s),links)
    if ARCHIVE_CHAT:
        archive=post_preview(s)+'\n\n'+'\n\n'.join(f'🔘 {b["label"]}\n{b["prompt"]}' for b in s['buttons'])
        send(ARCHIVE_CHAT,archive)
