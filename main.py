...
PERMISSIONS = ['prompts','prices','vip','orders','vip_orders','reports','ads','training','admins','stats','notifications','settings','customer_data']
DEFAULT_DATA = {
    'prompts': {},
    'vip': {},
    'orders': [],
...
def private_info_text(o):
    ...
    if prompt: lines += ['', '📝 پرامپت/اطلاعات سفارش:', prompt]
    return '\n'.join(lines)

def channel_order_keyboard(oid):
    # This button opens the bot. The bot itself authorizes owner/Admin Plus before revealing private data.
    if BOT_USERNAME:
        return kb([[urlbtn('👤 گرفتن آیدی و پرامپت',f'https://t.me/{BOT_USERNAME}?start=orderinfo_{oid}')]])
    return kb([])
...
def admin_permissions_page(cid,aid):
    ...
    perm_labels={'prompts':'پرامپت‌ها','prices':'قیمت‌ها','vip':'VIP','orders':'سفارشات','vip_orders':'سفارشات VIP','reports':'گزارش مشکلات','ads':'تبلیغات','training':'آموزش','admins':'ادمین‌ها','stats':'آمار','notifications':'اعلان‌ها','settings':'تنظیمات','customer_data':'اطلاعات خصوصی مشتری'}
...
def finish_order(uid):
    ...
    if ORDERS_CHAT:
        ...
        if s.get('ref_photo') and s.get('receipt'):
            ...
            if ids:
                # Telegram albums cannot have inline buttons; add the secure button to a caption/info message only if needed.
                r2=send(ORDERS_CHAT,details,channel_order_keyboard(oid))
...
def order_action_kb(oid):
    ...
    if BOT_USERNAME and st not in {'🟢 تمام شد','❌ رد شده'}:
        rows.append([urlbtn('👤 گرفتن آیدی و پرامپت',f'https://t.me/{BOT_USERNAME}?start=orderinfo_{oid}')])
    return kb(rows)
...
def can_manage_order(uid,o):
    ...
    return has_perm(uid,'vip_orders' if o.get('type')=='vip' else 'orders')
