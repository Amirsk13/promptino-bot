...
def copy_rows(text):
    # Telegram limits CopyTextButton.text to 256 characters. Long prompts must be
    # split into copyable chunks; otherwise Telegram rejects the whole message.
    text=text or ''
    if not text: return [[{'text':'📋 پرامپت خالی است'}]]
    chunks=[text[i:i+256] for i in range(0,len(text),256)]
    if len(chunks)==1:
        return [[{'text':'📋 کپی پرامپت','copy_text':{'text':chunks[0]}}]]
    return [[{'text':f'📋 کپی بخش {i+1}/{len(chunks)}','copy_text':{'text':chunk}} for i,chunk in enumerate(chunks)]]

def delivery_kb(v):
    rows=copy_rows(v.get('prompt',''))
