...
def publish_testimonial_album(uid,kind,number,photos):
    ...
    if kind=='vip':
        ...
        if contact:
            ...
            if BOT_USERNAME:
                send(TESTIMONIALS_CHAT,'🔐 اطلاعات مشتری VIP',kb([[urlbtn('👤 آیدی مشتری',f'https://t.me/{BOT_USERNAME}?start=vipcust_{oid}')]]))
...
def finish_order(uid):
    ...
    if s.get('type')=='customer':
        item='سفارش ساخت عکس با پرامپت مشتری'
        model=''
    elif s.get('type')=='logo':
        item='سفارش ساخت لوگو'
        model=''
    else:
        pid=s.get('prompt_ids',[''])[0]
        vk=(s.get('selected_variant') or (pid,''))[1]
        variant=DATA.get('prompts',{}).get(pid,{}).get('variants',{}).get(vk,{})
        model=variant.get('label',vk)
        item=f'{pid[1:]}'
    ...
    if ORDERS_CHAT:
        ...
        if s.get('receipt'):
            send_photo(ORDERS_CHAT,s['receipt'],details,order_action_kb(oid))
        else:
            send(ORDERS_CHAT,details,order_action_kb(oid))
        if s.get('ref_photo'):
            send_photo(ORDERS_CHAT,s['ref_photo'],f'📸 عکس مرجع سفارش {oid}')
...
def order_action_kb(oid):
    ...
    if o.get('payment')=='pending' and st not in {'❌ رد شده','🟢 تمام شد'}:
        ...
        if o.get('type') in {'customer','logo'}:
            rows.append([btn('🆔 آیدی برای تحویل دادن',f'delivery_id|{oid}')])
        else:
...
def handle_state_message(uid,cid,m):
    ...
    if typ=='vip_add':
        ...
        if step=='prompt':
            ...
            if ARCHIVE_CHAT:
                send(ARCHIVE_CHAT,f'👑 {vip_label(key,DATA["vip"][key])}\n\n🆔 {key}\n\n{ text }')
    ...
    if typ=='vip_order':
        ...
        if step=='receipt':
            ...
            for a in ADMIN_IDS:
                if s.get('receipt'): send_photo(a,s['receipt'],vip_details,order_action_kb(oid))
                else: send(a,vip_details,order_action_kb(oid))
            if ORDERS_CHAT:
                if s.get('receipt'): send_photo(ORDERS_CHAT,s['receipt'],vip_details,order_action_kb(oid))
                else: send(ORDERS_CHAT,vip_details,order_action_kb(oid))
...
def callback(uid,cid,data,msgid=None):
    ...
    if data.startswith('vip_customer|'):
        if not is_owner(uid): return send(cid,'⛔ این اطلاعات فقط برای مالک ربات قابل مشاهده است.')
        oid=data.split('|',1)[1]; ref=DATA.get('vip_customer_refs',{}).get(oid)
        if not ref: return send(cid,'❌ اطلاعات مشتری این سفارش پیدا نشد.')
        if BOT_USERNAME:
            send(uid,f'👤 {ref.get("vip_label",oid)}\n📱 آیدی/شماره ثبت‌شده مشتری: {ref.get("contact","")}',kb([[urlbtn('🤖 ورود به ربات',f'https://t.me/{BOT_USERNAME}?start=vipcust_{oid}')]]))
        else:
    ...
    if data.startswith('pay_ok|') or data.startswith('pay_bad|'):
        ...
        return
    if data.startswith('delivery_id|'):
        oid=data.split('|',1)[1]; o=find_order(oid)
        if not o: return
        if not can_manage_order(uid,o): return send(cid,'⛔ دسترسی نداری.')
        o['status']='🔵 تحویل داده شد'; o['delivery_contact']=o.get('contact',''); save_data(f'Delivery contact {oid}')
        send(cid,f'🆔 آیدی/شماره برای تحویل سفارش {oid}:\n📱 {o.get("contact","")}\n\n📌 این اطلاعات را خودت در پیوی مشتری بفرست. ربات چیزی را در کانال سفارشات ارسال نمی‌کند.')
        edit_order_message(cid,msgid,o) or send(cid,view_order_text(o),order_action_kb(oid))
        return
    if data.startswith('status|'):
        ...
        if not can_manage_order(uid,o): return send(cid,'⛔ دسترسی نداری.')
        if st=='delivered':
            o['status']='🔵 تحویل داده شد'; save_data(f'Status {oid}'); deliver_order_to_user(o)
        elif st=='satisfied':
