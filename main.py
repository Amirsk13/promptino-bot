...
def order_action_kb(oid):
    ...
    if st not in {'🟢 تمام شد','❌ رد شده'}:
        ...
        # Private customer data is available after payment approval. The contact
        # button remains available through delivery until satisfaction.
        if o.get('payment')=='approved':
            private_prompt=private_prompt_button_for_order(o)
            if private_prompt: rows.append([private_prompt])
            private_logo=private_logo_button_for_order(o)
            if private_logo: rows.append([private_logo])
    return kb(rows)
...
def callback(uid,cid,data,msgid=None,callback_id=None):
    # Answer the exact callback query that triggered this action. Do not suppress
    # clicks by time: every distinct click must advance the order exactly one step.
    answer(callback_id or CURRENT_CB)
...
def handle_update(update):
    ...
    # If a user is in an active flow, consume the message there first.
    # This prevents contact/receipt/photo messages from falling through to the
    # global /start/menu router and makes the order flow resilient to accidental
    # /start text or Telegram client quirks.
    if uid in STATES:
