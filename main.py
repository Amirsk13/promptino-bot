import os, json, base64, requests, re, threading
...
def private_button_for_order(o):
    ...
    typ=o.get('type')
    label={'vip':'👤 گرفتن آیدی VIP','customer':'👤 گرفتن آیدی و پرامپت مشتری','logo':'👤 گرفتن آیدی و توضیحات لوگو','normal':'👤 گرفتن آیدی و پرامپت'} .get(typ,'👤 گرفتن اطلاعات مشتری')
    return urlbtn(label,f'https://t.me/{BOT_USERNAME}?start=orderinfo_{o.get("id")}')

def channel_order_keyboard(oid):
    o=find_order(oid)
    b=private_button_for_order(o) if o else None
    return kb([[b]]) if b else kb([])
        ...
        if o.get('channel_expires_at') and o.get('channel_message_ids'):
            ...
            if expired and not o.get('channel_deleted'):
                for mid in o.get('channel_message_ids',[]):
                    if ORDERS_CHAT and mid:
                        api('deleteMessage',{'chat_id':ORDERS_CHAT,'message_id':mid})
...
def repair_existing_order_messages():
    # Repair older two-photo orders created by the buggy version: they had the
    # details twice and no action buttons. Keep the album and replace the extra
    # details message with a button-only action message.
    changed=False
    for o in DATA.get('orders',[]):
        mids=o.get('channel_message_ids') or []
        if not mids or not ORDERS_CHAT: continue
        if o.get('type') in {'customer','logo','normal'} and len(mids)>=3 and o.get('ref_photo') and o.get('receipt'):
            # Last message was the duplicated details message in the old version.
            api('deleteMessage',{'chat_id':ORDERS_CHAT,'message_id':mids[-1]})
            r=send(ORDERS_CHAT,'🔘 عملیات سفارش',order_action_kb(o.get('id')))
            if r.get('ok'):
                o['channel_message_ids']=mids[:-1]+[r.get('result',{}).get('message_id')]
                changed=True
        elif len(mids)==1:
            # Re-attach the current action buttons to legacy single-photo/text orders.
            if edit_order_message(ORDERS_CHAT,mids[0],o): changed=True
    if changed: save_data('Repair existing order channel messages')
...
def order_text(uid,text):
    ...
    if s['step']=='what':
        ...
        for k,v in DATA['prompts'][pid].get('variants',{}).items():
            rows.append([btn(f'{pid[1:]} | {v.get("label",k)}',f'ord_variant|{pid}|{k}')])
        rows.append([btn('❌ لغو','ord_cancel')]); s['step']='variants'
...
def finish_order(uid):
    s=STATES[uid]; oid=next_id('customer' if s.get('type')=='customer' else 'logo' if s.get('type')=='logo' else 'order'); now=datetime.now().strftime('%Y-%m-%d %H:%M')
    if s.get('type')=='customer':
        item='سفارش ساخت عکس با پرامپت مشتری'; model=''
    elif s.get('type')=='logo':
    ...
    o={'id':oid,'type':s['type'],'user_id':uid,'contact':s['contact'],'item':item,'model':model,
       ...
       'payment':'pending','created_at':now}
    DATA['orders'].append(o); save_data(f'New order {oid}')
    if ORDERS_CHAT:
        details=f'🆔 {oid}\n🛒 سفارش: {item}\n'
        if model: details+=f'🤖 مدل: {model}\n'
        details+=f'💰 قیمت: {money(o["price"])} تومان\n👤 واریزکننده: {s.get("payer_name","")}\n🕐 {now}\n📌 وضعیت: {o["status"]}'
        # Keep customer ID/contact out of the channel. A secure bot deep-link is the only way to reveal it.
        if s.get('ref_photo') and s.get('receipt'):
            # Telegram albums cannot carry an inline keyboard. Put the details only
            # once in the album caption, then send a button-only action message.
            media=[{'type':'photo','media':s['receipt']},{'type':'photo','media':s['ref_photo'],'caption':details}]
            r=api('sendMediaGroup',{'chat_id':ORDERS_CHAT,'media':media})
            ids=[x.get('message_id') for x in r.get('result',[])] if r.get('ok') else []
            if ids:
                r2=send(ORDERS_CHAT,'🔘 عملیات سفارش',order_action_kb(oid))
                if r2.get('ok'): ids.append(r2.get('result',{}).get('message_id'))
            o['channel_message_ids']=ids
        elif s.get('receipt'):
            r=send_photo(ORDERS_CHAT,s['receipt'],details,order_action_kb(oid))
            if r.get('ok'): o['channel_message_ids']=[r.get('result',{}).get('message_id')]
        else:
            r=send(ORDERS_CHAT,details,order_action_kb(oid))
            if r.get('ok'): o['channel_message_ids']=[r.get('result',{}).get('message_id')]
        # Keep private-contact order types in the channel for 14 days, then remove them automatically.
        if o.get('type') in {'vip','customer','logo'}:
            o['channel_expires_at']=(datetime.now()+timedelta(days=ORDER_RETENTION_DAYS)).isoformat()
            save_data(f'Channel retention {oid}')
...
def order_action_kb(oid):
    ...
    if o.get('payment')=='pending' and st not in {'❌ رد شده','🟢 تمام شد'}:
        ...
        if o.get('type') in {'customer','logo'}:
            rows.append([btn('🔵 تحویل داده شد',f'status|{oid}|delivered')])
        else:
            rows.append([btn('🔵 تحویل داده شد',f'status|{oid}|delivered')])
    elif st=='🔵 تحویل داده شد':
        rows.append([btn('🟢 رضایت دریافت شد',f'status|{oid}|satisfied')])
    if st not in {'🟢 تمام شد','❌ رد شده'}:
        rows.append([btn('💬 پیام به مشتری',f'msg_user|{oid}')])
    if BOT_USERNAME and st not in {'🟢 تمام شد','❌ رد شده'}:
        pb=private_button_for_order(o)
...
def orders_admin(cid, vip_only=False, order_type=None):
    if order_type:
        ...
        items=[o for o in DATA.get('orders',[]) if o.get('type')=='vip']
    else:
        items=[o for o in DATA.get('orders',[]) if o.get('type')!='vip']
...
def publish_post(uid):
    ...
    if ARCHIVE_CHAT:
        ...
        if not album_result.get('ok'):
            ...
            for b in s['buttons']:
                # Send the variant title separately from the prompt itself.
                send(ARCHIVE_CHAT,f'🔘 {b["label"]}')
                # Always keep the complete prompt in one message. Telegram's
                # CopyText button is limited to 256 chars, so only add it when
                # the whole prompt fits; never split the prompt into parts.
                prompt=b.get('prompt','')
                if prompt and len(prompt)<=256:
                    send(ARCHIVE_CHAT,prompt,kb([[{'text':'📋 کپی پرامپت','copy_text':{'text':prompt}}]]))
                else:
                    send(ARCHIVE_CHAT,prompt)
...
def handle_state_message(uid,cid,m):
    ...
    if typ=='vip_order':
        ...
        if step=='receipt':
            ...
            o={'id':oid,'type':'vip','user_id':uid,'contact':s['contact'],'item':s['vip'],'price':s['price'],'payer_name':s.get('payer_name',''),'status':'🟠 در حال تأیید','payment':'pending','created_at':now}
            DATA['orders'].append(o); save_data(f'VIP order {oid}')
            vip_details=(f'👑 سفارش VIP {oid}\n{vip_label(s["vip"],DATA["vip"][s["vip"]])}\n💰 {money(s["price"])} تومان\n👤 واریزکننده: {s.get("payer_name","")}\n🕐 {now}\n📌 وضعیت: {o["status"]}')
            if ORDERS_CHAT:
                r=send_photo(ORDERS_CHAT,s['receipt'],vip_details,order_action_kb(oid)) if s.get('receipt') else send(ORDERS_CHAT,vip_details,order_action_kb(oid))
                if r.get('ok'): o['channel_message_ids']=[r.get('result',{}).get('message_id')]
                o['channel_expires_at']=(datetime.now()+timedelta(days=ORDER_RETENTION_DAYS)).isoformat()
...
def edit_order_message(cid,msgid,o):
    ...
    r=api('editMessageText',{'chat_id':cid,'message_id':msgid,'text':view_order_text(o),'reply_markup':order_action_kb(o.get('id'))})
    if r.get('ok'): return True
    # Single-photo order messages are captions, not text messages.
    r=api('editMessageCaption',{'chat_id':cid,'message_id':msgid,'caption':view_order_text(o),'reply_markup':order_action_kb(o.get('id'))})
...
def callback(uid,cid,data,msgid=None):
    answer(CURRENT_CB)
    # Ignore duplicate Telegram webhook deliveries for the same action for a few seconds.
    import time
    action_key=(uid,data)
    now_ts=time.time()
    last_ts=RECENT_ACTIONS.get(action_key,0)
    if now_ts-last_ts < 4:
        return
    RECENT_ACTIONS[action_key]=now_ts
    for k,t in list(RECENT_ACTIONS.items()):
        if now_ts-t > 30: RECENT_ACTIONS.pop(k,None)
    ...
    if data=='testimonial_start' and (guard(uid,'orders') or guard(uid,'vip_orders')): testimonial_admin(cid); STATES[uid]={'type':'testimonial','step':'kind','photos':[]}; return
    if data.startswith('testimonial_type|') and (guard(uid,'orders') or guard(uid,'vip_orders')):
        kind=data.split('|',1)[1]; STATES[uid]={'type':'testimonial','step':'number','kind':kind,'photos':[]}; send(cid,'🔢 فقط عدد سفارش را بفرست. مثال: 2'); return
    ...
    if data=='logo_order': start_order(uid,logo=True); return
    ...
    if data.startswith('vip_buy|'):
        ...
        STATES[uid]={'type':'vip_order','step':'contact','vip':k,'price':price_for(k)}
    ...
    if data.startswith('pay_ok|') or data.startswith('pay_bad|'):
        ...
        ok=data.startswith('pay_ok'); o['payment']='approved' if ok else 'rejected'
        if ok:
            o['status']='🟡 در حال انجام'
            user_msg=f'✅ پرداخت سفارش {oid} تأیید شد.\n\n🟡 سفارش شما تأیید شد و در حال انجام است.'
            admin_msg='✅ پرداخت تأیید شد و وضعیت سفارش به «🟡 در حال انجام» تغییر کرد.'
        else:
            o['status']='❌ رد شده'
            user_msg=f'❌ پرداخت سفارش {oid} رد شد.\n\nاگر اشتباهی رخ داده، از بخش «⚠️ گزارش مشکل» گزارش بده.'
            admin_msg='❌ پرداخت رد شد و وضعیت سفارش «❌ رد شده» شد.'
        ...
        if not ok:
            ...
            edit_order_message(cid,msgid,o) or send(cid,view_order_text(o),order_action_kb(oid))
    ...
    if data.startswith('status|'):
        ...
        if not can_manage_order(uid,o): return send(cid,'⛔ دسترسی نداری.')
        if st=='delivered':
            o['status']='🔵 تحویل داده شد'; save_data(f'Status {oid}')
            ...
            DATA.setdefault('completed_orders',[]).append(deepcopy(o))
            # Keep only lightweight completed history for stats/testimonial lookup; remove from active order panels.
            save_data(f'Satisfaction {oid}')
            DATA['orders']=[x for x in DATA.get('orders',[]) if x.get('id')!=oid]
            save_data(f'Archive completed {oid}')
            send(cid,f'🟢 رضایت سفارش {oid} ثبت شد. وضعیت: تمام شد')
    ...
    if data in {'price_channel','price_customer','price_logo'} and guard(uid,'prices'):
        ...
        STATES[uid]={'type':'price','step':'value','key':key}; send(cid,f'💰 قیمت جدید را برای {"ساخت عکس با پرامپت کانال" if key=="channel_prompt" else "ساخت عکس با پرامپت مشتری" if key=="customer_prompt" else "سفارش ساخت لوگو"} به تومان بفرست.'); return
    ...
    if data=='vip_add' and guard(uid,'vip'):
        STATES[uid]={'type':'vip_add','step':'title','buttons':[]}
