...
PROMPTINO_CHAT=os.getenv('PROMPTINO_CHAT',PROMPTINO_CHANNEL).strip()
...
def next_id(kind):
 ...
 return ('ORD' if kind=='order' else 'VIP') + f"-{DATA['counters'][kind]}"
...
def publish_post(uid):
 ...
 r=send_photo(PROMPTINO_CHAT,s['photo'],post_preview(s),kb(rows))
...
def reports_admin(cid):
 rows=[]
 for r in DATA.get('reports',[])[-20:]: rows.append([btn(f"{r['id']} | {r['status']}",f'report_view|{r["id"]}')])
 send(cid,'⚠️ گزارش مشکلات',kb(rows+[[btn('🔙 مدیریت','admin_menu')]]))
