...
def order_action_kb(oid):
    ...
    if st not in {'🟢 تمام شد','❌ رد شده'}:
        private_contact=private_button_for_order(o)
        if private_contact: rows.append([private_contact])
        private_prompt=private_prompt_button_for_order(o)
        if private_prompt and o.get('payment')=='approved': rows.append([private_prompt])
    return kb(rows)
...
def view_order_text(o):
    # Customer contact is deliberately omitted from order/channel messages.
    # It is retrieved privately through the bot by authorized admins.
    text=f'🆔 {o["id"]}\n🛒 {o.get("item","")}\n'
...
def handle_update(update):
    ...
    if uid in STATES and not text.startswith('/start'):
