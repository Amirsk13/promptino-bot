...
def publish_vip_testimonial(o):
    if not TESTIMONIALS_CHAT: return
    key=o.get('item','')
    v=DATA.get('vip',{}).get(key,{})
    text=(f'👑 رضایت سفارش VIP\n\n{vip_label(key,v)}\n'
          f'🟢 سفارش با موفقیت تکمیل شد.')
    send(TESTIMONIALS_CHAT,text,kb([[btn('👤 آیدی مشتری',f'vip_customer|{o.get("id")}')]]))

def purge_vip_order(oid):
    orders=DATA.get('orders',[])
    before=len(orders)
    DATA['orders']=[o for o in orders if o.get('id')!=oid]
    if len(DATA['orders'])!=before: save_data(f'Cleanup VIP order {oid}')

def schedule_vip_cleanup(oid):
    try:
        t=threading.Timer(300, purge_vip_order, args=(oid,)); t.daemon=True; t.start()
    except Exception as e: print('VIP cleanup timer',e)

def cleanup_vip_orders():
    now=datetime.now()
    changed=False; kept=[]
    for o in DATA.get('orders',[]):
        if o.get('type')=='vip' and o.get('status')=='🟢 تمام شد' and o.get('satisfied_at'):
            try:
                if (now-datetime.fromisoformat(o['satisfied_at'])).total_seconds()>=300:
                    changed=True; continue
            except Exception: pass
        kept.append(o)
    if changed:
        DATA['orders']=kept; save_data('Cleanup old VIP orders')
...
def callback(uid,cid,data,msgid=None):
    ...
    if data.startswith('vip_customer|'):
        ...
        send(uid,f'👑 {ref.get("vip_label","VIP")}\n🆔 آیدی مشتری: {ref.get("user_id")}')
        return
    ...
    if data.startswith('status|'):
        ...
        if st=='delivered':
            if o.get('type') in {'customer','logo'}:
                STATES[uid]={'type':'deliver_media','step':'photo','oid':oid,'msgid':msgid,'chat_id':cid}
                send(cid,f'📦 سفارش {oid} آماده تحویل است.\n\n📸 عکس نهایی/فایل تحویلی را همین‌جا ارسال کن تا برای مشتری فرستاده شود.\nبرای لغو: /cancel')
                return
            ...
            if o.get('type')=='vip':
                DATA.setdefault('vip_customer_refs',{})[oid]={'vip_label':vip_label(o.get('item',''),DATA.get('vip',{}).get(o.get('item',''),{})),'user_id':o.get('user_id')}
            ...
            if o.get('type')=='vip':
                publish_vip_testimonial(o); schedule_vip_cleanup(oid)
        else:
            return
        if find_order(oid):
            edit_order_message(cid,msgid,o) or send(cid,view_order_text(o),order_action_kb(oid))
