...
def finish_order(uid):
    ...
    o={'id':oid,'type':s['type'],'user_id':uid,'contact':s['contact'],'item':item,'model':model,
       ...
       'payer_name':s.get('payer_name',''),'ref_photo':s.get('ref_photo'),'status':'🟡 در حال انجام',
...
def handle_state_message(uid,cid,m):
    ...
    if typ=='vip_order':
        ...
        if step=='receipt':
            ...
            o={'id':oid,'type':'vip','user_id':uid,'contact':s['contact'],'item':s['vip'],'price':s['price'],'payer_name':s.get('payer_name',''),'status':'🟡 در حال انجام','payment':'pending','created_at':now}
...
def view_order(cid,oid):
    ...
    send(cid,text,order_action_kb(oid))
...
def callback(uid,cid,data,msgid=None):
    ...
    if data.startswith('pay_ok|') or data.startswith('pay_bad|'):
        ...
        if ok:
            ...
            admin_msg='✅ پرداخت تأیید شد و سفارش در حال انجام است.'
        else:
            user_msg=f'❌ پرداخت سفارش {oid} تأیید نشد.\n\nاگر فکر می‌کنی اشتباهی رخ داده، از بخش «⚠️ گزارش مشکل» گزارش بده.'
            admin_msg='❌ پرداخت رد شد.'
        save_data(f'Payment {oid}')
        send(o['user_id'],user_msg); send(cid,admin_msg); return
    if data.startswith('status|'):
        ...
        if o:
            o['status']=labels.get(st,st); save_data(f'Status {oid}')
            send(cid,f'✅ وضعیت: {o["status"]}')
            if st=='delivered': deliver_order_to_user(o)
        return
